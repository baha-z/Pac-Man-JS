...
//check collisions with ghosts function
...
function render() {
    ...
    if (life > 0 && dottotal > 0) {
        ...
        ctx.fillStyle = "white";
        if (dottotal <= 0) {
            ctx.fillText("You Win!", (canvas.width / 2.8), (canvas.height / 2.5));
        ...
        //Retry option
        ctx.fillText("Retry?: Y/N", (canvas.width / 2.8), (canvas.height / 1.5));
        // if yes, reset game
        if (event.keyCode == 89) {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            score = 0;
